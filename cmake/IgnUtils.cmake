...
#################################################
# ign_build_executable(<target>
#                      SOURCES <sources>
#                      [LIB_DEPS <library_dependencies>]
#                      [INCLUDE_DIRS <include_dependencies>]
#                      [EXCLUDE_PROJECT_LIB])
#
# Build executable for an ignition project. Arguments are as follows:
#
# TARGET: Required. The name of the target executable.
#
# SOURCES: Required. The names (without a path) of the source files for your
#          executable.
#
# PREFIX: Optional. This will append <prefix> onto the executable name.
#
# LIB_DEPS: Optional. Additional library dependencies that executable
...
#               to this executable .
#
# EXCLUDE_PROJECT_LIB: Pass this argument if you do not want your executable to
...
#
macro(ign_build_executable target)
  #------------------------------------
  # Define the expected arguments
  ...
  #------------------------------------
  # Parse the arguments
  _ign_cmake_parse_arguments(ign_build_executable "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

  add_executable(${target} ${ign_build_executable_SOURCES})

  if(NOT ign_build_executable_EXCLUDE_PROJECT_LIB)
    target_link_libraries(${target} ${PROJECT_LIBRARY_TARGET_NAME})
  endif()

  if(ign_build_executable_LIB_DEPS)
    target_link_libraries(${target} ${ign_build_executable_LIB_DEPS})
  endif()
  ...
  target_include_directories(${target}
    PRIVATE
      ${PROJECT_SOURCE_DIR}
      ${PROJECT_BINARY_DIR}
      ${ign_build_executable_INCLUDE_DIRS})
...
macro(ign_build_executables)
  ...
  foreach(exec_file ${ign_build_executables_SOURCES})
    ...
    set(EXCLUDE_PROJECT_LIB "")
    if (ign_build_executables_EXCLUDE_PROJECT_LIB)
      set(EXCLUDE_PROJECT_LIB "EXCLUDE_PROJECT_LIB")
    endif()

    ign_build_executable(
      ${BINARY_NAME}
      SOURCES ${exec_file}
      LIB_DEPS ${ign_build_executables_LIB_DEPS}
      INCLUDE_DIRS ${ign_build_executables_INCLUDE_DIRS}
      ${EXCLUDE_PROJECT_LIB}
    )
...
#################################################
# ign_build_test(TARGET <target>
#                 SOURCES <sources>
#                 [LIB_DEPS <library_dependencies>]
#                 [INCLUDE_DIRS <include_dependencies>]
#                 [TEST_LIST <output_var>])
#
# Build test for an ignition project. Arguments are as follows:
#
# TARGET: Required. Name of the test.
#         Preferably prefixed with type, eg:
#           UNIT, INTEGRATION, PERFORMANCE, or REGRESSION.
#
...
macro(ign_build_test target)
  ...
  #------------------------------------
  # Parse the arguments
  _ign_cmake_parse_arguments(ign_build_test "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

  if(BUILD_TESTING)
    set(EXCLUDE_PROJECT_LIB "")
    if (ign_build_test_EXCLUDE_PROJECT_LIB)
      set(EXCLUDE_PROJECT_LIB "EXCLUDE_PROJECT_LIB")
    endif()

    ign_build_executable(
      "${target}"
      SOURCES ${ign_build_test_SOURCES}
      LIB_DEPS ${ign_build_test_LIB_DEPS}
      INCLUDE_DIRS ${ign_build_test_INCLUDE_DIRS} ${PROJECT_SOURCE_DIR}/test/gtest/include
      ${EXCLUDE_PROJECT_LIB}
    )
    ...
    if(USE_LOW_MEMORY_TESTS)
      target_compile_options(${target} PRIVATE -DUSE_LOW_MEMORY_TESTS=1)
    endif()

    add_test(
      NAME ${target}
      COMMAND ${target} --gtest_output=xml:${CMAKE_BINARY_DIR}/test_results/${target}.xml)
    ...
    set_tests_properties(${target} PROPERTIES TIMEOUT 240)

    if(PYTHONINTERP_FOUND)
      # Check that the test produced a result and create a failure if it didn't.
      # Guards against crashed and timed out tests.
      add_test(check_${target} ${PYTHON_EXECUTABLE} ${IGNITION_CMAKE_TOOLS_DIR}/check_test_ran.py
        ${CMAKE_BINARY_DIR}/test_results/${target}.xml)
    endif()
  endif()
...
macro(ign_build_tests)
  ...
  if(BUILD_TESTING)
    ...
    set(EXCLUDE_PROJECT_LIB "")
    if (ign_build_tests_EXCLUDE_PROJECT_LIB)
      set(EXCLUDE_PROJECT_LIB "EXCLUDE_PROJECT_LIB")
    endif()

    foreach(exec_file ${ign_build_tests_SOURCES})
      get_filename_component(BINARY_NAME ${exec_file} NAME_WE)
      set(BINARY_NAME ${TEST_TYPE}_${BINARY_NAME})

      ign_build_test(${BINARY_NAME}
        SOURCES ${exec_file}
        LIB_DEPS ${ign_build_tests_LIB_DEPS}
        INCLUDE_DIRS ${ign_build_tests_INCLUDE_DIRS}
        ${EXCLUDE_PROJECT_LIB}
      )

      if(ign_build_executables_EXEC_LIST)
        list(APPEND ${ign_build_tests_EXEC_LIST} ${BINARY_NAME})
      endif()
